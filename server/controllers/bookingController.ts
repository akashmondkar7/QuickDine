...
export const createBooking = async (req:AuthRequest,res:Response):Promise<void>=> {
    try {
        const{restaurant:restaurantId,date,time,guests,occasion,specialRequests}=req.body;
        ...
        if( requestedGuests > availableSeats){
            res.status(400).json({message:
                `Unable to reserve.Only ${availableSeats} sets are available for this time slot.`
            });
            return;
        }
        ...
        //Populate restaurant info before returing
...
export const getMyBookings = async (req:AuthRequest,res:Response):Promise<void>=> {
    try {
        const booking = await Booking.find({user:req.user?._id}).populate("restaurant","name location image address slug").sort({date:-1,time:-1});
        res.json(booking);
    } catch (error:any) {
        console.error(error);
        res.status(400).json({message:error.message});
